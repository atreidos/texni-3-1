...
## 27.03.2026 — структурированные логи Edge (`_shared/logger.ts`)

| Проверка | Результат |
|----------|-----------|
| Пароли / JWT / raw Authorization в логах | Не пишутся; мутации без тела; `business.context` санитизируется по списку ключей |
| Секреты в .md | Не добавлялись; описан только флаг `EDGE_STRUCTURED_LOGGING` |
| Отключение логов | Через секрет `EDGE_STRUCTURED_LOGGING=false` (или 0/no/off) |
| Поведение API / авторизация | Логика запросов не менялась, только обёртка и замена `console.error` |
...
| Секреты | Секреты не в логах / не в ответах клиенту | ✅ Структурированные логи без тел запросов и заголовков; ошибки — JSON без утечки тел; клиенту — короткие сообщения |
