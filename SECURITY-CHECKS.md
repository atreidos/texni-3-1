...
| Проверка | Результат |
|----------|-----------|
| Секреты / JWT в коде счётчика | Не добавлялись; только публичный ID счётчика |
| Куда уходят данные | Запросы к `mc.yandex.ru` (просмотры, вебвизор по настройкам счётчика в кабинете Метрики) |
| Запись в БД | Не затронута — только клиентский скрипт |
| Согласие пользователя | На стороне продукта/юрисдикции при необходимости |
