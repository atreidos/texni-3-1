# DocFlow — SQL-схема для Supabase

Готовый скрипт для вставки в **Supabase SQL Editor**.  
Порядок выполнения: функции → таблицы → триггеры → RLS → индексы.

> `current_user_plan()` и `has_plan()` написаны на `plpgsql` (не `sql`), чтобы тело функции не проверялось до первого вызова — это позволяет объявить их до создания таблицы `profiles`.
...
CREATE INDEX IF NOT EXISTS idx_document_fields_group
  ON public.document_fields(group_name);
...
| Таблица | Описание |
|---------|----------|
| `profiles` | Профиль пользователя, тариф, счётчик документов |
| `organizations` | Организации (ООО/ИП) с реквизитами |
| `documents` | Документы (PDF/DOCX) с привязкой к организации |
| `document_fields` | Поля документа (ключ-значение с типом) |
| `payments` | История платежей (только чтение для пользователя) |

## Ключевые решения

- **`payments` INSERT только через `service_role`** — платёж создаётся на бэкенде после подтверждения от платёжного шлюза, не фронтендом напрямую.
- **`docs_used` обновляется триггером** — автоматически при вставке/удалении документа.
- **ИИ-заполнение** — ограничение по тарифу реализуется на уровне бэкенда через `has_plan(ARRAY['pro','business'])`. RLS не блокирует SELECT/UPDATE полей, но API-endpoint проверяет тариф перед вызовом ИИ.
- **Анонимный режим** — только фронтенд, в БД не пишет. Все таблицы требуют `auth.uid() IS NOT NULL` через RLS.
