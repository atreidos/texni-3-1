...
└── functions/
    ...
    ├── dadata-find-party/
    │   └── index.ts          # прокси к DaData API: поиск организации по ИНН (JWT, DADATA_API_KEY)
...
| Реквизиты по ИНН (DaData) | по клику «Заполнить автоматически» | `/functions/v1/dadata-find-party` |
...
- **DaData**: кнопка «Заполнить автоматически» рядом с полем ИНН вызывает Edge Function `dadata-find-party`, которая проксирует запрос к DaData API. Заполняются реквизиты (name, inn, kpp, ogrn, address, phone, email); банковские поля остаются пустыми. При `data.state.status !== "ACTIVE"` или `data.invalid === true` показываются жёлтые предупреждения, сохранение не блокируется.
