...
│
├── lib/
│   └── supabase.js        # Supabase client (singleton), fetchFromEdge/invokeEdgeFunction/callEdgeFunction.
│                          # Для write (create/update/delete): invokeEdgeFunction — токен из сессии клиента.
│
├── context/
...
│                          # Важно: в onAuthStateChange не использовать await Supabase-методов — deadlock
│
├── data/
...
└── pages/
    ...
    ├── EditorPage.jsx     # /editor — публичная; Save: upload в Storage + documents-create
    ...
    └── dashboard/
        ...
        ├── DocumentsPage.jsx      # fetch documents via Edge; delete, download (signedUrl)
        ...
        └── SettingsPage.jsx       # update profile через invokeEdgeFunction('profile-update'); supabase.auth.updateUser (пароль)
```
...
│   └── index.ts          # прокси к DaData API: поиск по ИНН (findById) и по названию (suggest/party)
    ...
    ├── documents-*/index.ts # documents list/create/delete + Storage
...
| Документы | fetch при монтировании | `/functions/v1/documents-list`; файлы в Storage (`documents` bucket) |
...
- **Скачать** — создаёт signed URL (60 с) и открывает в новой вкладке. Для документов без `filePath` кнопка неактивна.
- **Удалить** — Edge Function удаляет файл из Storage и запись из БД.

### EditorPage (сохранение документа)

- **Сохранить** (только для авторизованных): загрузка файла в `supabase.storage.from('documents')` по пути `{user_id}/{doc_id}/{filename}`, затем вызов `documents-create` с метаданными. При успехе — редирект на `/dashboard/documents`.
...
| Файл | Сценарий |
|------|----------|
| 01-регистрация-и-редирект-в-личный-кабинет | Регистрация с валидными данными → редирект на /dashboard |
| 02-вход-и-редирект-на-обзор | Логин test@example.com → страница «Обзор» |
| 03-неавторизованный-редирект-на-логин | Защищённые маршруты перенаправляют на /auth/login |
| 04-валидация-формы-регистрации | Пустые поля, короткий пароль, пароли не совпадают, без оферты |
| 05-создание-организации | Поиск через DaData (Яндекс, Озон), сохранение, проверка в списке |
| 06-удаление-организации-и-документа | Удаление org/doc через UI, проверка исчезновения |
| 07-обновление-профиля | Настройки: имя, телефон, сохранение |

**auth.fixture.js** — хелпер `login(page)` для входа под test@example.com. Тестовый пользователь создаётся в Supabase Dashboard → Users → Add user.
