...
- **Структурированные логи бэкенда (Edge):** реализованы в `_shared/logger.ts` (JSON в stdout). События POST/PATCH/DELETE — метод, path, статус, длительность; ошибки — сообщение и стек; бизнес-события — после успешных мутаций (организации, документы, профиль, платёж, DaData). Регистрация пользователя идёт через Supabase Auth на клиенте, отдельной Edge Function «signUp» в репо нет, поэтому отдельного `business`-события «registration» в этих функциях нет — при необходимости подключают Auth Hook / логи Supabase. Отключение всех таких логов: секрет `EDGE_STRUCTURED_LOGGING=false` (или `0` / `no` / `off`).
