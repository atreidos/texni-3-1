...
На публичном фронтенде подключён счётчик Яндекса (ID в `frontend/index.html`): отправка просмотров, карта кликов, отказоустойчивый подсчёт, Вебвизор. Данные обрабатываются Яндексом по [их политике](https://yandex.ru/legal/confidential/). Учитывайте требования к согласию на cookies/ПДн для вашей юрисдикции. Секреты приложения в Метрику не передаются.
